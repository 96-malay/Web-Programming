...
function validateWebsite(website) {
    ...
    let websiteRegex = /^https?:\/\/(www\.){1}[-a-zA-Z0-9@:%_\+~#=]{5,}\.com$/   //[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)/
...
async  getAll() {
    ...
    //if (infoList === null) throw 'This collection has no data.'
...
async  rename(id, newWebsite) {
    ...
    const restaurantsCollection = await restaurants();
    let objInfo = await restaurantsCollection.findOne({'_id':parsedId})
    if(objInfo === null) throw `No data for ID: ${id}` 
