...
function checkStockId(Stockid) {
    ...
    if( typeof(Stockid) !== 'string') throw [400,'Input Stock ID is not a string']
   // Stockid = Stockid.trim()
    if( ! Stockid.replace(/\s/g,'').length) throw [400,'Input Stock ID is the Blank String']
    if( Stockid.length === 0) throw [400,'Stock ID is Empty']
    return true
